...
def load_sidebar(user):
    ...
    links = Links.objects.filter(article=None, is_enable=True).order_by('sequence')
...
def load_article_metas(article, user, open_site_comment):
    """
    获得文章meta信息
    :param article:
    :return:
...
def load_article_detail(article, isindex, user, forListPage=False):
    """
    加载文章详情
    :param article:
...
def gravatar_url(email, size=40):
    ...
    if cache.get(cachekey):
        ...
        default = "https://xxxxxxxxxxxxxx/image/2017/03/26/120117.jpg".encode('utf-8')
